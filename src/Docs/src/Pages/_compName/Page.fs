    ...
    Html.div [
        ...
        prop.children (
            ...
            |> List.map (fun (x, types) ->
                Html.div [
                    prop.className "flex flex-col gap-3 max-w-4xl"
                    prop.children [
                        Shadcn.separator []
                        ...
                        for type' in types do
                            Shadcn.badge [
                                ...
                                prop.children [
                                    Html.span [
                                        prop.className "font-mono font-bold text-sm"
                                        prop.text $"Shadcn.%s{x} %s{type'}"
                                    ]
                                ]
                            ]
                        ...
                        let props =
                            apiReferencesProps
                            |> Map.tryPick (fun name props ->
                                if x.ToLowerInvariant().StartsWith(name.ToLowerInvariant()) then
                                    Some props
                                else
                                    None)
                            |> Option.defaultValue []
